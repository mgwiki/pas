...
void c_sha256_add_s1(value b, value s){
  CAMLparam2(b, s);
  secp256k1_sha256_write(ShaB_val(b), (const unsigned char*)(String_val(s)), 1);
  CAMLreturn0;
}

void c_sha256_add_s4(value b, value s){
  CAMLparam2(b, s);
  secp256k1_sha256_write(ShaB_val(b), (const unsigned char*)(String_val(s)), 4);
  CAMLreturn0;
}

void c_sha256_add_s32(value b, value s){
  CAMLparam2(b, s);
  secp256k1_sha256_write(ShaB_val(b), (const unsigned char*)(String_val(s)), 32);
  CAMLreturn0;
}
...
value c_sha256_finalize(value b){
  ...
  secp256k1_sha256_finalize(ShaB_val(b), (unsigned char*)(String_val(r)));
  CAMLreturn(r);
}
...
value c_sha256_bebits(value s){
  ...
  secp256k1_sha256_write(&hasher, (const unsigned char*)(String_val(s)), caml_string_length(s));
  CAMLlocal1(r);
  r = caml_alloc_string(32);
  secp256k1_sha256_finalize(&hasher, (unsigned char*)(String_val(r)));
  CAMLreturn(r);
}
...
value c_sha256_list_bebits(value l){
  ...
  for (; l != Val_emptylist; l = tl(l))
    secp256k1_sha256_write(&hasher, (const unsigned char*)(String_val(hd(l))), caml_string_length(hd(l)));
  CAMLlocal1(r);
  r = caml_alloc_string(32);
  secp256k1_sha256_finalize(&hasher, (unsigned char*)(String_val(r)));
  CAMLreturn(r);
